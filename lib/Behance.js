...
class Behance {
	...
	static async user(username, detailed) {
		...
		return MainMethods_1.MainMethods.getJSONFromUrl(
			`https://www.behance.net/${username}`,
		).then((jsonResponse) => {
			// then return more data
			if (detailed) {
				return MainMethods_1.MainMethods.deleteInsignificantUserData(
					jsonResponse,
				).then(() => {
					return jsonResponse;
				});
	...
	static async project(path) {
		...
		return MainMethods_1.MainMethods.getJSONFromUrl(
			...
			return MainMethods_1.MainMethods.deleteInsignificantProjectData(
				jsonResponse,
			).then(() => {
				return jsonResponse;
			});
	...
	static async randomProject(sort, time) {
		...
		// get JSON data with jsonDOM()
		return MainMethods_1.MainMethods.getJSONFromUrl(URL).then(
			(jsonResponse) => {
